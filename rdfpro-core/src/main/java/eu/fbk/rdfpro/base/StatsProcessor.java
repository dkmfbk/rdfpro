...
public final class StatsProcessor extends RDFProcessor {
    ...
    static StatsProcessor doCreate(final String... args) {
        final Options options = Options.parse("", args);

        String namespace = options.getOptionArg("n", String.class);
        if (namespace != null && !namespace.contains(":")) {
            final String prefixOrNamespace = namespace;
            namespace = Statements.PREFIX_TO_NS_MAP.get(prefixOrNamespace);
            if (namespace == null) {
                throw new IllegalArgumentException("Invalid namespace: " + prefixOrNamespace);
            }
        }

        final URI property = options.getOptionArg("p", URI.class);
        final URI context = options.getOptionArg("c", URI.class);
        final Long threshold = options.getOptionArg("t", Long.class);
        final boolean processCooccurrences = options.hasOption("o");

        return new StatsProcessor(namespace, property, context, threshold, processCooccurrences);
    }

    public StatsProcessor(@Nullable final String outputNamespace,
    ...
    private final class Handler extends HandlerBase {
        ...
        public void handleStatement(final Statement statement) throws RDFHandlerException {
            ...
            if (o instanceof URI
                    && p.equals(StatsProcessor.this.sourceProperty)
                    && (StatsProcessor.this.sourceContext == null || Objects.equals(c,
                            StatsProcessor.this.sourceContext))) {
                ...
                for (final Map.Entry<SourceStats, PartialStats> e : this.directBlockStats
                    ...
                    if (s.tss != null) {
                        for (final TypeStats ts : s.tss) {
                            ...
                            if (StatsProcessor.this.processCooccurrences) {
        ...
        private void emitStatistics() throws RDFHandlerException {
            ...
            for (final SourceStats s : this.sourceList) {
                ...
                final String label = Statements.formatValue(uri).replace("<", "").replace(">", "")
            ...
            for (final TypeStats ts : this.typeList) {
                ...
                if (p0.entities < StatsProcessor.this.threshold) {
                    continue;
                }
                final String label = Statements.formatValue(ts.type).replace("<", "")
                        .replace(">", "")
                        + " (" + p0.entities + ")";
            ...
            for (final PropertyStats ps : this.propertyList) {
                final PropertyStats.Partition p0 = ps.partitions[0];
                if (p0.triples < StatsProcessor.this.threshold) {
                    continue;
                }
                ...
                final String label = String.format("%s (%d, %s%s%s)",
                        Statements.formatValue(ps.property).replace("<", "").replace(">", ""),
                ...
                for (int i = 0; i < ps.partitions.length; ++i) {
                    ...
                    if (p != null && p.triples >= StatsProcessor.this.threshold) {
                        final URI source = this.sourceList.get(i).source;
                        final URI spURI = spURIs.get(source);
            ...
            for (final URI term : VOID.TERMS) {
                emit(term, VOIDX.LABEL, Statements.formatValue(term));
            }
        ...
        private URI mintURI(final URI... inputURIs) {
            ...
            if (StatsProcessor.this.outputNamespace != null) {
                builder.append(StatsProcessor.this.outputNamespace);
            } else {
