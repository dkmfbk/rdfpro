...
public final class TboxProcessor extends RDFProcessor {

    private static final Logger LOGGER = LoggerFactory.getLogger(TboxProcessor.class);

    static TboxProcessor doCreate(final String... args) {
        Options.parse("", args);
    ...
    public RDFHandler getHandler(@Nullable final RDFHandler handler) {
        ...
        return new Handler(Handlers.decouple(sink));
    }
        ...
        public void endRDF() throws RDFHandlerException {
            ...
            if (LOGGER.isInfoEnabled()) {
                ...
                for (final Map.Entry<URI, Term> entry : this.terms.entrySet()) {
                    ...
                    if (term.isLanguage) {
                        if (term.isUsed) {
                            final String s = Statements.formatValue(uri);
                            if (term.isProperty) {
