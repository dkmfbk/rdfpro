...
public final class SmushProcessor extends RDFProcessor {

    private static final Logger LOGGER = LoggerFactory.getLogger(SmushProcessor.class);

    private final long bufferSize;

    private final String[] rankedNamespaces;

    static SmushProcessor doCreate(final String... args) {
        final Options options = Options.parse("s!|*", args);
        final String[] namespaces = options.getPositionalArgs(String.class).toArray(new String[0]);
        for (int i = 0; i < namespaces.length; ++i) {
            final String namespaceOrPrefix = namespaces[i];
            if (!namespaceOrPrefix.contains(":")) {
                namespaces[i] = Statements.PREFIX_TO_NS_MAP.get(namespaceOrPrefix);
                if (namespaces[i] == null) {
                    throw new IllegalArgumentException("Invalid namespace: " + namespaceOrPrefix);
                }
            }
        }
        final Long bufferSize = options.getOptionArg("s", Long.class);
        return new SmushProcessor(bufferSize, namespaces);
    }

    public SmushProcessor(final long bufferSize, final String... rankedNamespaces) {
    ...
    public RDFHandler getHandler(final RDFHandler handler) {
        ...
        return new Handler(sink, this.bufferSize, this.rankedNamespaces);
    }
